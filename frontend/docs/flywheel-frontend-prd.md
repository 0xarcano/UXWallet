# Flywheel Protocol Frontend — Product Requirements Document
...
Flywheel is a non-custodial wallet and aggregated liquidity protocol. Users delegate assets via the **Flywheel Wallet**; the **Flywheel Solver** fulfills intents using the **Aggregated Liquidity Pool** (user-delegated assets + Flywheel Treasury). Rewards from intent fulfillment are split **50% to Users** and **50% to the Flywheel Treasury**. User funds are always protected.

The frontend serves as the **Flywheel Wallet** UI: it keeps the security boundary behind a single delegation signature, hides the complexity of bridging/gas/chain-switching, and presents the user with a unified yield-bearing balance. The goal is to make common transactions simple and atomic from the user's perspective (one-step).
...
The **Flywheel Solver** runs as a Just-In-Time bot at off-chain speed using Yellow Technology for the LI.FI marketplace plus the **Aggregated Liquidity Pool** (user-delegated liquidity across multiple chains in Yellow Custody / Nitrolite) working together to generate yield. When the **pool has liquidity** on the target chain, the liquidity layer fulfills (best fee); when it does not, the system creates **intent orders in the LiFi marketplace** (funded from pool on source chains or Treasury-sponsored).

### Flywheel Wallet

Non-custodial, one-step liquidity unification, automated yield-bearing, one-step gasless Yellow L3 transactions, one-step cross-chain transactions with the best fee rates on the market.

### How it works

1. **Delegation & Pool:** User delegates assets; App + ClearNode set up Nitrolite session/channel; funds locked in Custody (Yellow Smart Account / Nitrolite). User grants Session Key so Flywheel Solver can fulfill intents.
2. **Settlement (Yellow / ERC-7824):** Off-chain state updates (Clearing Engine, state channel); on-chain payout via Adjudicator `conclude` / `transfer` (Custody). Rewards 50% User, 50% Flywheel Treasury.
3. **When pool cannot fulfill:** System creates intent orders in the **LiFi marketplace**. Funds come from **pool on source chains** (release to LiFi solver) or **Flywheel Treasury** (sponsored same-chain). User balance debited; third party receives on target chain.
...
Web3 wallets today impose unacceptable friction on users. Flywheel addresses six core pain points:

| # | Pain Point | Impact | Flywheel Solution |
|---|-----------|--------|-------------------|
...
| 5 | **No passive yield** | Idle assets in wallets earn nothing | Flywheel Solver activities generate yield; 50% distributed to users, 50% to Flywheel Treasury |
...
- [ ] After delegation, no further wallet pop-ups are required for internal operations (rebalancing, Flywheel Solver fulfillment)
...
- [ ] The Flywheel Treasury sponsors the LI.FI bridge fee transparently
...
> As a new user, I want a guided onboarding that explains Flywheel and gets me set up quickly.
...
> As a user, I want to add liquidity into Flywheel (deposit/unify) with a clear preview of amounts, fees, and outcomes.
...
- **Onboarding/Sign in**
  - Show Flywheel logo and connect button.
  - **Unify form (Deposit)**
...
- **Flywheel Solver scope:** The Persistent Session Key grants the Flywheel Solver permission to use liquidity to fill LI.FI marketplace orders only. Liquidity does not move to other wallets — it only changes chain localization via intent order fulfillment; ownership is guaranteed to the original user.
- **User Funds vs Treasury:** User funds are always protected and never used for owner withdrawals. The Flywheel Treasury receives 50% of rewards, serves as part of pool liquidity, and only Treasury may be withdrawn by system owners.
...
- **Yellow / ERC-7824 settlement**: state-channel lifecycle states that the UI must represent (open/update/close, etc.); Custody Contract and Adjudicator.
- **Delegation / Persistent Session Key**: typed-data schema (application, allowances, expires_at), verification, and revocation mechanism exposed to the frontend.
- **Flywheel Solver**: Evaluates and fulfills intents from the Aggregated Liquidity Pool; falls back to LiFi when pool insufficient.
- **Vault liquidity signals**: ability to determine "Direct Exit" vs "Sponsored Exit (Hybrid)" conditions for withdrawals.
- **lif-rust**: Stateless LiFi integration microservice (`POST /lifi/quote`, `POST /intent/build`, `POST /intent/calldata`); mocked in Phase 1, real in Phase 2.
...
- **Unification Intent (ERC-7683)**: Intent-based action (via LI.FI) that unifies fragmented balances into Flywheel vaults.
...
- **Hybrid Exit / Sponsored Exit**: Withdrawal mode where the Flywheel Treasury sponsors LI.FI bridge fees when the local vault is not enough to fulfill the requested amount in the selected token, providing a "Fast Exit Guarantee".
...
- **Flywheel Solver**: The Just-In-Time solver bot that fulfills intents using the Aggregated Liquidity Pool; falls back to LiFi marketplace when pool cannot fulfill.
- **Flywheel Treasury**: Receives 50% of intent-fulfillment rewards; serves as part of the Aggregated Liquidity Pool; only withdrawable by system owners.
- **Aggregated Liquidity Pool**: Combined user-delegated assets + Flywheel Treasury used by the Flywheel Solver to fulfill intents.
- **Custody Contract**: On-chain contract (Yellow Smart Account / Nitrolite) where delegated funds are locked.
- **Adjudicator**: On-chain contract used for dispute resolution and Force Withdrawal; users can present last signed state to claim funds if backend/ClearNode is unavailable.
- **Force Withdrawal**: Safety mechanism allowing users to present their last signed state to the on-chain Adjudicator to claim funds when backend/ClearNode is unavailable.
